...
def testcase_rag():
    ...
    func_name = testcase_rag.__name__.replace("_", "-")
    model: ChatOpenAI = TongyiModelManager.create_chat_model(func_name)
    vector: Chroma = ChromaVectorStoreManager.create_vector_store()
    ...
    input_chain: RunnableParallel[str] = RunnableParallel(
        ...
        context=retriever | RunnableLambda(lambda documents: parse_docs(documents))
    )
